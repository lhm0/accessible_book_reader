...
constexpr unsigned long kBusyWaitTimeoutMs = 100;
constexpr unsigned long kRfSwitchTimeoutMs = 100;

bool waitForPinLevel(uint8_t pin, int expectedLevel, unsigned long timeoutMs) {
...
bool PN5180::readRegister(uint8_t reg, uint32_t *value) {
  ...
  SPI.beginTransaction(PN5180_SPI_SETTINGS);
  const bool ok = transceiveCommand(cmd, 2, (uint8_t*)value, 4);
  SPI.endTransaction();
 ...
 */
 bool PN5180::writeEEPROM(uint8_t addr, uint8_t *data, int len) {
   ...
   SPI.beginTransaction(PN5180_SPI_SETTINGS);
   const bool ok = transceiveCommand(buffer, len+2);
   SPI.endTransaction();
...
bool PN5180::readEEprom(uint8_t addr, uint8_t *buffer, int len) {
  ...
  SPI.beginTransaction(PN5180_SPI_SETTINGS);
  const bool ok = transceiveCommand(cmd, 3, buffer, len);
  SPI.endTransaction();
...
uint8_t * PN5180::readData(int len, uint8_t *buffer /* = NULL */) {
  ...
  SPI.beginTransaction(PN5180_SPI_SETTINGS);
  const bool ok = transceiveCommand(cmd, 2, buffer, len);
  SPI.endTransaction();
...
bool PN5180::loadRFConfig(uint8_t txConf, uint8_t rxConf) {
  ...
  SPI.beginTransaction(PN5180_SPI_SETTINGS);
  const bool ok = transceiveCommand(cmd, 3);
  SPI.endTransaction();
...
bool PN5180::setRF_on() {
  ...
  SPI.beginTransaction(PN5180_SPI_SETTINGS);
  const bool ok = transceiveCommand(cmd, 2);
  SPI.endTransaction();
  ...
  while (0 == (TX_RFON_IRQ_STAT & getIRQStatus())) { // wait for RF field to set up
    if (millis() - start >= kRfSwitchTimeoutMs) {
      return false;
    }
    delay(1);
  }
...
bool PN5180::transceiveCommand(uint8_t *sendBuffer, size_t sendBufferLen, uint8_t *recvBuffer, size_t recvBufferLen) {
  ...
  // 0.
  if (!waitForPinLevel(PN5180_BUSY, LOW, kBusyWaitTimeoutMs)) {
    return false;
  }
  // 1.
  ...
  // 3.
  if (!waitForPinLevel(PN5180_BUSY, HIGH, kBusyWaitTimeoutMs)) {
