...
class PN532_I2C : public PN532Interface {
  ...
  private:
    ...
    int16_t requestChunk(uint8_t* buf, uint8_t len, uint16_t timeout);
    int16_t findStartCode(const uint8_t* buf, uint8_t len);
    bool isAckFrame(const uint8_t* buf, uint8_t len, uint8_t start);
